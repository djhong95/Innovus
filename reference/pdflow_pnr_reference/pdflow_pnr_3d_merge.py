...
def write_tcl(filename: str, mmmc_file: Union[str, None], cpf_file: Union[str, None], upf_file: Union[str, None],
              options_file: str) -> int:
    '''
    ...
    '''
    ret = 0
    merge_result_dir = os.path.join(FLOW_ENVS['WORK_AREA'], 'pnr', '3d_merge', 'results')
    ...
    netlist_file = os.path.join(merge_result_dir, flow_file_utils.join_filename(FLOW_ENVS['BLOCK'], 'input', 'v'))
    pnr_3d_route_tcl.write('set init_verilog %s' % (netlist_file))
    pdflow_pnr.pnr_input_collaterals.append(netlist_file)

    lef_files = [os.path.join(merge_result_dir, flow_file_utils.join_filename(FLOW_ENVS['BLOCK'], 'lef'))]
    ...
    input_floorplan_def_file = os.path.join(merge_result_dir, flow_file_utils.join_filename(FLOW_ENVS['BLOCK'], 'input', 'floorplan', 'def'))
    ...
    if pdflow.is_compact2D_flow():
        ...
        if FLOW_ENVS['IMPL_TYPE'] == 'f2b':
            nanoroutemode_opts['-drouteOnGridOnly'] = 'via ' + str(bottom_routing_layer) + ':' + str(
                total_num_route_layer)
            pass
        else:
            nanoroutemode_opts['-drouteOnGridOnly'] = 'via ' + str(bottom_routing_layer) + ':' + str(
                total_num_route_layer - bottom_routing_layer + 1)
    ...
    if pdflow.is_compact2D_flow():
        ...
        rcout_opts = {
            '-rc_corner': '$rc_corner',
            '-spef': os.path.join(merge_result_dir, flow_file_utils.join_filename(FLOW_ENVS['BLOCK'], '$rc_corner', 'spef', 'gz'))
        }
    ...
    defout_opts = {
        '-floorplan': None,
        '-routing': None,
        os.path.join(merge_result_dir, flow_file_utils.join_filename(FLOW_ENVS['BLOCK'], 'def', 'gz')): None,
    }
    # pnr_3d_route_tcl.write_cmd_w_opts('defOut', defout_opts)
    # pnr_3d_route_tcl.write('saveNetlist %s' % (os.path.join(merge_result_dir, flow_file_utils.join_filename(FLOW_ENVS['BLOCK'], 'v', 'gz'))))
...
# def main(main_tcl_file: str, in_DEF0: str, in_DEF1: str) -> int:
def main(main_tcl_file: str) -> int:
    ...
    if pdflow.is_3D_design():
        ...
        finish_result_dir = os.path.join(FLOW_ENVS['WORK_AREA'], 'pnr', '3d_finish', 'results')
        part0_def_file = os.path.join(finish_result_dir, flow_file_utils.join_filename('part0', 'def', 'gz'))
        part1_def_file = os.path.join(finish_result_dir, flow_file_utils.join_filename('part1', 'def', 'gz'))
        ...
        for pvt in pvts:
            ...
            lib_pvt_dir = os.path.join(route_result_dir, 'lib', pvt)
        ...
        # load mmmc file
        route_script_dir = os.path.join(FLOW_ENVS['WORK_AREA'], 'pnr', '3d_route', 'scripts')
        if pdflow.is_compact2D_flow():
            mmmc_file = os.path.join(route_script_dir, flow_file_utils.join_filename(FLOW_ENVS['BLOCK'], 'mmmc', 'tcl'))
        else:
            mmmc_file = None
        flow_log_utils.write_subsubsection_comment(logger, 'generate innovus scripts')

        # load upf file
        upf_file = os.path.join(route_script_dir, flow_file_utils.join_filename(FLOW_ENVS['BLOCK'], 'upf'))

        # load options file
        options_file = os.path.join(route_script_dir, flow_file_utils.join_filename(FLOW_ENVS['BLOCK'], 'options', 'tcl'))

        # generate 3d design def, verilog
        ipartDEF = pdflow_lefdef_utils.DEF(lef=iLEF)
        ipartDEF.read_def(part0_def_file)
        ipartDEF.read_def(part1_def_file)
        ...
        i3DDEF = ipartDEF.make_3d_def(FLOW_ENVS['IMPL_TYPE'], i3DLEF, maps_2d_to_3d, ['part0', 'part1'], FLOW_ENVS['BLOCK'], True, True)
        ...
        pdflow_pnr.pnr_input_collaterals.append(main_tcl_file[0])

        return ret
    else:
        logger.error('3d_merge stage is not supported in 2d designs')
