...
def write_tcl(filename: str):
	'''
	write a script to perform route stage of pnr

	:param filename: path to pnr script
	:return: 0 if script is generated successfully. Otherwise, 1
	'''
	ret = 0
	pnr_route_tcl = flow_tcl_utils.TclFile(filename=filename)

	pdflow_pnr.innovus_start_commands(pnr_route_tcl)

	if is_on(FLOW_CFGS['INNOVUS']['FEATURES'], 'route_opt_design'):
		pnr_route_tcl.write_section_comment('route_opt_design options')
		...
		pnr_route_tcl.write_cmd_w_opts('setExtractRCMode', extractrcmode_opts)

		analysismode_opts = {
			'-analysisType': 'onChipVariation',
			'-cppr': 'both',
		}
		pnr_route_tcl.write_cmd_w_opts('setAnalysisMode', analysismode_opts)

		delaycalmode_opts = {
			'-siAware': 'true',
			'-engine': 'aae',
		}
		pnr_route_tcl.write_cmd_w_opts('setDelayCalMode', delaycalmode_opts)

		optmode_opts = {
			'-addInstancePrefix': 'routeopt_'
		}
		pnr_route_tcl.write_cmd_w_opts('setOptMode', optmode_opts)

		pnr_route_tcl.write_section_comment('run route_opt_design')
		...
		pnr_route_tcl.write_cmd_w_opts('route_opt_design', route_opt_design_opts)
	else:
		pnr_route_tcl.write_section_comment('route design')
		pnr_route_tcl.write('routeDesign')

	# store verilog & sdf
	pnr_route_tcl.write('saveNetlist %s' % (os.path.join(FLOW_ENVS['RESULT_DIR'], flow_file_utils.join_filename(FLOW_ENVS['BLOCK'], 'v', 'gz'))))
	pnr_route_tcl.write('write_sdf %s' % os.path.join(FLOW_ENVS['RESULT_DIR'], flow_file_utils.join_filename(FLOW_ENVS['BLOCK'], 'sdf')))

	pdflow_pnr.innovus_end_commands(pnr_route_tcl)
	pnr_route_tcl.close()
...
def main(main_tcl_file: str) -> int:
	'''
	write all the scripts required for route stage of pnr
